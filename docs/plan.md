...
## Adendo — Passo 5b: Montagem de edição (Digest) e dois modos de criação (Compose)

> Adicionado após validação inicial do MVP (Passos 1-7 já implementados). Refina comportamento que o plano original deixava implícito. Ver ADR 0029 e ADR 0030.

**Objetivo**: fechar o que acontece depois da aprovação de artigos no Digest, e adicionar um segundo modo de criação no Compose.

**Entregáveis — Digest**:
- Inspeção do schema já implementado para reconciliar nomes de tabela antes de criar migration nova (ADR 0029)
- Migration: entidade de edição de newsletter + relacionamento com os itens aprovados que a compõem
- Endpoint/caso de uso: "montar edição" (botão manual) — dispara geração de rascunho (intro + resumo dos aprovados) via `LLMClient`, reaproveitando a fila assíncrona (ADR 0028)
- UI: botão "Montar edição", tela de edição do rascunho gerado (TipTap), exportação manual (ADR 0003)

**Entregáveis — Compose**:
- Dois pontos de entrada ao criar artigo novo: "Gerar com IA" (fluxo já existente) e "Começar em branco" (editor vazio)
- Ações de IA sob demanda no modo em branco (sugerir tópico, gerar rascunho, expandir trecho selecionado), via fila assíncrona (ADR 0028), nunca automáticas
- Campo `origin` em `generated_content` (opcional, recomendado) para diferenciar origem do conteúdo

**Pronto quando**: um artigo aprovado no Digest consegue virar uma edição de newsletter revisável e exportável via botão manual; e o Compose permite tanto gerar um artigo completo via IA quanto começar do zero com IA sob demanda.
